...
export const middlewareNewUser: any = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (nome && altura && nascimento && cidade) {
        ...
        return res.status(400).json({ message: "erro ao receber os dados 'nome'" })

    } else if (!altura) {
        return res.status(400).json({ message: "erro ao receber os dados 'altura'" })

    } else if (!nascimento) {
        return res.status(400).json({ message: "erro ao receber os dados 'nascimento'" })
    } else if (!cidade) {
        return res.status(400).json({ message: "erro ao receber os dados 'cidade'" })

    } else {
        return res.status(400).json({ message: "erro ao receber os dados" })
    }
}
...
export const middlewareCheckId: any = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (checkID == "1" || checkID == "2") {
        ...
        return res.status(400).json({ message: "Você não é o admin" })
    }
...
export const middlewarNewProduct = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    if (nome && preco && quantidade && categoria) {
        ...
        return res.status(400).json({ message: "erro ao receber o nome" })

    } else if (!preco) {
        return res.status(400).json({ message: "erro ao receber o preco" })
    } else if (!quantidade) {
        return res.status(400).json({ message: "erro ao receber o quantia" })
    } else if (!categoria) {
        return res.status(400).json({ message: "erro ao receber o quantia" })
    } else {
        return res.status(400).json({ message: "erro ao receber os dados" })
...
export const middlewareCheckProduto = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    if (!nome || !preco || !quantidade || !categoria) {
        return res.status(400).json({ message: "Atualize os dados corretamente" })
